...
if len(defs) != 1:
    sys.exit("expected exactly one top-level _get_ple_embedding_quant_method after the shim, found %d" % len(defs))
body = ast.get_source_segment(src, defs[0])
if "PLE_QUANT_OVERRIDE" not in body or "Qwen3_8FlashNextPLEFp8EmbeddingMethod()" not in body:
    sys.exit("the surviving resolver is not the shim")
print("shim is the active resolver")
'
