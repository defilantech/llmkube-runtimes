...
The base is `nightly-20260823`; the newest b12x on that date was 1.2.6, so the upgrade is doing real work. It is installed `--no-deps`, which is safe for a checked reason rather than by habit: b12x's requirement set is byte-identical across 1.2.6, 1.2.8 and 1.3.0, so nothing needs resolving and torch/vLLM cannot be swapped underneath a CUDA stack that is already correct for `sm_121`. The gate checks this by diffing `pip check` against a baseline the Dockerfile records *before* the install, so a future bump that **does** add a dependency fails the build rather than a GB10.

That diff is not defensive over-engineering; a bare `pip check` fails here. The base ships `nvidia-cutlass-dsl` **4.7.0** while b12x — at every version, the pin has not moved — requires exactly **4.6.2**. So does `quack-kernels` 0.6.4, a base package this image never touches, which is what proves the conflict is inherited rather than introduced by the `--no-deps` install. It is also empirically non-fatal: this is the community's working GB10 stack, so the metadata pin is stricter than the runtime requirement. (Note this is the `nvidia-cutlass-dsl` *Python* package, a different thing from vLLM's C++ `CUTLASS_REVISION`.) Pinning it down to 4.6.2 would change a CUDA DSL out from under a vLLM build that works today; dropping the check would give up the guard entirely. Diffing keeps the guard and tolerates the inheritance.
