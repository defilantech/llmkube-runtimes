...
A bare `pip check` **fails** on this image: it carries `nvidia-cutlass-dsl` **4.7.0** while b12x pins `==4.6.2`. That reads like the `--no-deps` install misfired. A plain baseline diff *also* fails, for the opposite reason: the base has no pip-visible b12x requirements at all (its baseline names only `quack-kernels`), so every b12x requirement is technically "new" the moment the wheel is installed.

What actually changed is **visibility, not content**. `cutlass-dsl` is 4.7.0 before and after; this layer never touches it. And `quack-kernels` 0.6.4 — a base package this image never touches — asks for 4.6.2 too, so 4.7.0 is a deliberate base choice made over an existing pin, not something introduced here.

So the cutlass-dsl family is allowlisted by name and printed loudly, while anything else must be inherited or absent. A future b12x that needs `flash-attn`, or that moves its `torch` floor, still fails the gate — which is the case `--no-deps` genuinely makes risky. The build also ships `/opt/llmkube/base-receipt.txt` recording what the base contained, because reasoning about the base from the outside already produced one wrong conclusion.
